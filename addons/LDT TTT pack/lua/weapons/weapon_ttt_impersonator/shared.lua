...
function SWEP:SecondaryAttack()
	...
	if SERVER then
		...
		if (ply:GetNWEntity("ImpTarget",IsEntity()):IsPlayer()) then
			if (ply:GetNWBool("Impersonate",false)) then
				ply:SetNWBool("Impersonate",false)
				local impname = ply:GetNWEntity("ImpTarget",IsEntity()):Nick()
				...
				local imp = ply:GetNWEntity("ImpTarget",IsEntity())
...
function SWEP:Equip()
	...
	if SERVER then
		...
		if (self.Weapon.ImpTarget != nil) and (self.Weapon.ImpName != nil) then
			...
			ply:SetNWBool("Impersonate",false)
			ply:SetNWEntity("ImpTarget",IsEntity())
