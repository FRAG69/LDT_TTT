   ...
   if SERVER then
      ...
      if self.DisarmCausedExplosion then
         ...
         r_outer = r_outer / 2.5
	  	  ...
	  	  AddToDamageLog({DMG_LOG.C4, "EXPLODE", dmgowner:Nick(), dmgowner:GetRoleString(), {dmgowner:SteamID()} })
...
if SERVER then
   ...
   function ENT:Disarm(ply)
      ...
      if ply != owner and IsValid(owner) then
         LANG.Msg(owner, "c4_disarm_warn")
	  	   ...
	  	   AddToDamageLog({DMG_LOG.C4, "DISARM", ply:Nick(), ply:GetRoleString(), owner:Nick(), {ply:SteamID(), owner:SteamID()} })
   ...
   function ENT:FailedDisarm(ply)
      ...
      SCORE:HandleC4Disarm(ply, self:GetOwner(), false)
	  	  ...
	  	  AddToDamageLog({DMG_LOG.C4, "FAILED", ply:Nick(), ply:GetRoleString(), self:GetOwner():Nick(), {ply:SteamID(), self:GetOwner():SteamID()}})
