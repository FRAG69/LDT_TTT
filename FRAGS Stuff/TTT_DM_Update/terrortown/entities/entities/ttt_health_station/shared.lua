   ...
   if self:GetStoredHealth() > 0 then
      ...
      if dmg > 0 then
         ...
         ply:SetHealth(new)
		  ...
		  if timer.Exists("Poison" .. ply:SteamID()) then
			timer.Destroy("Poison" .. ply:SteamID())
		 end
...
function ENT:OnTakeDamage(dmginfo)
   ...
   if IsPlayer(att) then
		AddToDamageLog({DMG_LOG.HEALTH_DMG, att:Nick(), att:GetRoleString(), self:GetPlacer():Nick(), dmginfo:GetDamage(), {att:SteamID(), self:GetPlacer():SteamID()}})
   ...
   if self:Health() < 0 then
	  ...
	  AddToDamageLog({DMG_LOG.HEALTH_DESTROY, att:Nick(), att:GetRoleString(), self:GetPlacer():Nick(), {att:SteamID(), self:GetPlacer():SteamID()}})
