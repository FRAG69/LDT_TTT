   ...
   if SERVER then
      ...
      elseif ent:GetClass() == "prop_ragdoll" then
         ...
         ent.burn_destroy = CurTime() + burn_time
			...
			if ent.was_role != nil and att != nil then
			 local roletext2 = nil
			 ...
			 local attname = att:Nick()
			 local attrole = att:GetRoleString()
			 AddToDamageLog({DMG_LOG.BODY_BURNT, attname, attrole, CORPSE.GetPlayerNick(ent, ""), roletext2, {att:SteamID(), CORPSE.GetPlayerID(ent, "")}})
		 end
...
function SWEP:PrimaryAttack()
   ...
   self:TakePrimaryAmmo( 1 )
    ...
    if SERVER and IsValid(self.Owner) then --record to player's last shot, for damage log use
		self.Owner.LastShot = { CurTime(), "a flare gun" }
   end
