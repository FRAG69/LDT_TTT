...
local function ColorForPlayer(ply)
   if IsValid(ply) then
      local c = namecolor.default
      if LDT_rankColors then
		 c= LDT_rankColors[ply:GetNWInt("LDT_rank")]
	  else
		 c = hook.Call("TTTScoreboardColorForPlayer", GAMEMODE, ply)
	  end
...
function PANEL:UpdatePlayerData()
   ...
   if ply:GetNWInt("LDT_rank") != 0 then
	self.cols[5]:SetText(LDT_ranknames[ply:GetNWInt("LDT_rank")])
	self.cols[5]:SetColor(LDT_rankColors[ply:GetNWInt("LDT_rank")])
   end
