...
function GM:HUDDrawTargetID()
   ...
   if ent:IsPlayer() then
      if ent:GetNWBool("disguised", false) or ent:GetNWBool("cloaked", false) then
         ...
         color = COLOR_RED
      elseif (ent:GetNWBool("Impersonate",false) and ent:GetNWEntity("ImpName","") != "") then
		 local imptar = ent:GetNWEntity("ImpTarget",IsEntity())
		 sb_tag = imptar.sb_tag
		 
         client.last_id = imptar
		 if (client.last_id == IsEntity()) then 
			client.last_id = nil
		 end
