...
function GM:PlayerDeath( victim, infl, attacker)
   ...
   --TTT Donator Freezecam (It's simple but effective, plus I wanted to have some fun with OBS_MODE shizz)
   if victim:IsDonator() then
   if IsValid(attacker) and attacker:IsPlayer() and attacker!=victim then
		timer.Simple(2, function() if !IsValid(victim) or victim:Alive() then return false end if attacker:IsValid() then victim:Spectate(OBS_MODE_FREEZECAM) victim:SpectateEntity(attacker) end end)
	end
	timer.Simple(5, function() if !IsValid(victim) or victim:Alive() then return false end if victim:GetObserverMode()==OBS_MODE_NONE or victim:GetObserverMode()==OBS_MODE_FREEZECAM then victim:Spectate(OBS_MODE_CHASE) victim:SpectateEntity(table.Random(team.GetPlayers(victim:Team()))) end end)
end
...
function GM:Tick()
   ...
   for i= 1, #plys do
      ...
      if tm == TEAM_TERROR and ply:Alive() then
         ...
         if ply:HasEquipmentItem(EQUIP_PARA) then
		    
			local Vel = ply:GetVelocity()
			if (Vel[3] < 0) then // if Vel's z is negative (ie, if falling)
			   local NewVel = Vector(Vel[1],Vel[2],math.max(Vel[3],-425))
