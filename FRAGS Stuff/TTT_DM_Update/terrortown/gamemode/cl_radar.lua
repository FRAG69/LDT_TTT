...
local function DrawTarget(tgt, size, offset, no_shrink, slam)
   ...
   if sz == size then
	  local text
	  if not slam then
			text = math.ceil(LocalPlayer():GetPos():Distance(tgt.pos))
	  else
			text = "SLAM"
	  end
      ...
      if tgt.t then
		 ...
		 if not slam then
			...
			text = ""
			if tgt.beampos != nil and tgt.beampos:Distance( LocalPlayer():GetPos() ) <= 355 then
				local beamp = tgt.beampos:ToScreen()
				local endp = tgt.endpos:ToScreen()
				if beamp.visible and endp.visible then
					local dist = tgt.beampos:Distance( LocalPlayer():GetPos() )
					local alpha = math.Clamp(355-dist,0,255)
					surface.SetDrawColor(255,0,0,alpha)
					surface.DrawLine(beamp.x, beamp.y, endp.x, endp.y)
				end
			end
		 end
...
function RADAR:Draw(client)
   ...
   if self.bombs_count != 0 and client:IsActiveTraitor() then
      ...
      for k, bomb in pairs(self.bombs) do
         DrawTarget(bomb, 24, 0, true, bomb.slam)
      end
   end
...
local function ReceiveSLAMWarn(um)
   local idx = um:ReadShort()
   local armed = um:ReadBool()
   ...
   if armed then
      ...
      RADAR.bombs[idx] = {pos=pos, t=etime, slam=true}
   else
      RADAR.bombs[idx] = nil
   end
...
usermessage.Hook("slam_warn", ReceiveSLAMWarn)
