...
--   dtab1:SetSize(650, 450)
   ...
   dtabsheet:AddSheet(T("report_tab_events"), dtabevents, "icon16/application_view_detail.png", false, false, T("report_tab_events_tip"))
    ...
    --Detailed events
   dtabdetailed = vgui.Create("DPanel", dtabsheet)
   dtabdetailed:StretchToParent(padding,padding,padding,padding)
   
    local btnLoad = vgui.Create("DButton", dtabdetailed)
   btnLoad:SetSize(0,0)
	btnLoad.Think = function()
		RunConsoleCommand("ttt_detailed_getdamagelog", "0")
		btnLoad:Remove()
   end
   
   detlist = setupDetailedEvents(dtabdetailed)
   
   detbackpanel = vgui.Create("DPanel", detlist)
   detbackpanel:StretchToParent(120,130,120,130)
   detbackpanel.Paint = function()
	draw.RoundedBox(6,0,0,detbackpanel:GetWide(), detbackpanel:GetTall(), Color(0,0,0,220))
   end
   
   local deteventslabel2 = vgui.Create("DLabel", detbackpanel)
   deteventslabel2:SetText("Loading detailed events...")
   deteventslabel2:SetFont("Fretta4")
   deteventslabel2:SizeToContents()
   local ly = detbackpanel:GetTall()/2
   deteventslabel2:SetPos(detbackpanel:GetWide()/2-deteventslabel2:GetWide()/2, ly-deteventslabel2:GetTall() + 5)
   deteventslabel2:SetColor(color_white)
   
   local deteventslabel = vgui.Create("DLabel", detbackpanel)
   deteventslabel:SetText("(may take a few seconds)")
   deteventslabel:SetFont("Fretta1")
   deteventslabel:SizeToContents()
   deteventslabel:SetPos(detbackpanel:GetWide()/2-deteventslabel:GetWide()/2, ly+deteventslabel:GetTall()/2 - 5)
   deteventslabel:SetColor(color_white)
   
    dtabsheet:AddSheet("Detailed Events", dtabdetailed, "icon16/book_open.png", false, false, "Detailed log of the round's events")
