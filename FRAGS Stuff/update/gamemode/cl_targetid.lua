...
function GM:HUDDrawTargetID()
   ...
   if ent:IsPlayer() then
      if (ent:GetNWBool("disguised", false) or ent:GetNWBool("Cloaked",false)) then
         client.last_id = nil
         ...
         color = COLOR_RED
      elseif (ent:GetNWBool("Impersonate",false) and ent:GetNWEntity("ImpTarget",NullEntity()) !=NullEntity()) then
         client.last_id = ent:GetNWEntity("ImpTarget",NullEntity())
		 if (client.last_id == NullEntity()) then 
			client.last_id = nil
		 end
		 
         if (client:IsTraitor() and ent:IsTraitor()) or client:IsSpec() then
            text = ent:Nick() .. " (IMPERSONATING)"
            color = COLOR_RED
         else
	        text = ent:GetNWEntity("ImpTarget",NullEntity()):Nick()
         end
	  else
         text = ent:Nick()
         client.last_id = ent
