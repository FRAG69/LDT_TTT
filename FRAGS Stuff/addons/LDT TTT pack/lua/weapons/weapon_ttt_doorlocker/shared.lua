...
function SWEP:PrimaryAttack()
	if (self.Weapon:Clip1() >= 1) then
		...
		if (IsValid(target.Entity) and (target.Locked == nil)) then
			if ((target.Entity:GetClass() == "func_door_rotating") or (target.Entity:GetClass() == "prop_door_rotating") or (target.Entity:GetClass() == "func_door")) then
				...
				local func = 	function() 
			        target.Entity.EmitSound,target.Entity = "doors/door_latch3.wav"
		        end
				
				local funky = 	function() 
			        self.RemoveVariable = self,target
		        end
				timer.Simple(30, func)
				timer.Simple(30, funky)
