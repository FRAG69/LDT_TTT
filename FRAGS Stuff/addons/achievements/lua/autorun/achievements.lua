...
function PANEL:CCUpdateAchievement( name, percent, percentText )
	...
	if ( !ach.Unlocked && percent >= 1 ) then
		...
		local say = GetConVarString( "achievements_chatmessage_fix" )
		if ( string.Trim( say ) != "" ) then
			RunConsoleCommand( "say", string.Replace( say, "%achievement%", name ) )
		end
	end
...
local function Initialize()
	...
	/*
	local function ResetAchievements(ply)

    local PrevVer = ply:GetProperty("AchVer")
    local CurVer = 1

    if (PrevVer == nil) or (PrevVer < CurVer) then
    ResetAchievements(ply)

    ply:SetProperty("AchVer",CurVer)
