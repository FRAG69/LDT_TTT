...
do
    local ACH = {}
    ACH.Type = ACHIEVEMENT_PROGRESS
    ACH.Description = "Play one round of LDT TTT!"
	ACH.Icon = "gui/achievements/ttt_pissed.png"
    ACH.Target = 1
	ACH.Rewards = { { ps_points = 100 } }

    function ACH:Initialize()
        hook.Add("TTTEndRound", "AchvPlayTTT1", function()
            for _, ply in pairs( player.GetAll() ) do
                self:AddPoint( ply, 1 )
            end
        end)
    ...
    achievements.Register( category, "playttt1", "Let's Do This!", ACH )
end
...
do
	...
	ACH.Icon = "gui/achievements/ttt_dedicated.png"
    ACH.Target = 100
	ACH.Rewards = { { ps_points = 500 } }
    ...
    achievements.Register( category, "playttt100", "Can't stop the TTT", ACH )
end