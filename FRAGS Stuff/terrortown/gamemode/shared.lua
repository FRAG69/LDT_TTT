...
if (SERVER) then AddCSLuaFile( "achievements.lua" ) end

achievements.CreateCategory( "AI Killing" )

achievements.AddCustomHook( "PlayerSay" )
achievements.AddCustomHook( "PlayerSpawnedProp" )
achievements.AddCustomHook( "Donated" )
...
ACH.Target = 0
function ACH:PlayerSpawnedProp( ply, model, ent )
	self:Award( ply )
end
achievements.Register( "#gamemode", "Prop Spawner", ACH )
...
local ACH = {}
ACH.Type = ACHIEVEMENT_ONEOFF
ACH.Description = "Thank you for Donating!"
ACH.Image = "gui/silkicons/wrench"
ACH.Target = 1
function ACH:Donated( ply )
    if ply:IsDonator() then
		self:Award( ply )
	end
end
achievements.Register( "#misc", "I'm a Donator!", ACH )
