...
$getAdmin = mysql_query("SELECT * FROM `fragstats` WHERE `isadmin` = '1'");
$multiResult = mysql_num_rows($getAdmin);
}

echo "<div id='primary_content'>";
if ($sb_enabled == true){
echo "Filter the Admin list based on the LDT tracker database below!";
...
include("./includes/config.php");
$player = mysql_query("SELECT * FROM `fragstats` WHERE `isadmin` = '1'");
...
if ($searchType == "sb"){	
$player = mysql_query("SELECT * FROM `fragstats` WHERE `steamid` = '$myAdmins'");
}
