...
$var1 = mysql_query('SELECT * FROM `fragstats');
$uniqueusers = mysql_num_rows($var1);

$innocent = mysql_query('SELECT SUM(innocenttimes) AS innocent_sum FROM fragstats');
$innocentarray = mysql_fetch_array($innocent);
$innocenttotal = $innocentarray['innocent_sum'];

$traitor = mysql_query('SELECT SUM(traitortimes) AS traitor_sum FROM fragstats');
$traitorarray = mysql_fetch_array($traitor);
$traitortotal = $traitorarray['traitor_sum'];

$detective = mysql_query('SELECT SUM(detectivetimes) AS detective_sum FROM fragstats');
$detectivearray = mysql_fetch_array($detective);
$detectivetotal = $detectivearray['detective_sum'];

$death = mysql_query('SELECT SUM(deaths) AS deaths_sum FROM fragstats');
$deatharray = mysql_fetch_array($death);
$deathtotal = $deatharray['deaths_sum'];

$kills = mysql_query('SELECT SUM(kills) AS kills_sum FROM fragstats');
$killsarray = mysql_fetch_array($kills);
$killstotal = $killsarray['kills_sum'];

$head = mysql_query('SELECT SUM(headshots) AS headshots_sum FROM fragstats');
$headarray = mysql_fetch_array($head);
$headtotal = $headarray['headshots_sum'];

$point = mysql_query('SELECT SUM(points) AS points_sum FROM fragstats');
$pointarray = mysql_fetch_array($point);
$pointtotal = $pointarray['points_sum'];

$time = mysql_query('SELECT SUM(playtime) AS playtime_sum FROM fragstats');
$timearray = mysql_fetch_array($time);
$timetotal = $timearray['playtime_sum'];

$topscore = mysql_query("SELECT nickname, maxfrags FROM `fragstats` ORDER BY `fragstats`.`maxfrags` DESC LIMIT 0, 1");
$topscorearray = mysql_fetch_array ( $topscore );
$topscorefinal = $topscorearray['maxfrags'];
$topscorenick = $topscorearray['nickname'];
}
else{
$top10Time = mysql_query("SELECT nickname, playtime FROM `fragstats` ORDER BY `fragstats`.`playtime` DESC LIMIT 0, 10 ");
$top10Score = mysql_query("SELECT nickname, maxfrags FROM `fragstats` ORDER BY `fragstats`.`maxfrags` DESC LIMIT 0, 10 ");
$top10Deaths = mysql_query("SELECT nickname, deaths FROM `fragstats` ORDER BY `fragstats`.`deaths` DESC LIMIT 0, 10 ");
$top10Kills = mysql_query("SELECT nickname, kills FROM `fragstats` ORDER BY `fragstats`.`kills` DESC LIMIT 0, 10 ");
$top10Head = mysql_query("SELECT nickname, headshots FROM `fragstats` ORDER BY `fragstats`.`headshots` DESC LIMIT 0, 10 ");
$top10Points = mysql_query("SELECT nickname, points FROM `fragstats` ORDER BY `fragstats`.`points` DESC LIMIT 0, 10 ");
$top10KDR = mysql_query("SELECT nickname, kills, deaths, (kills / deaths) KDR FROM `fragstats` WHERE deaths >= '$deathLimit' AND playtime >= '$hourLimit' ORDER BY `KDR` DESC LIMIT 0, 10");

$rounds = mysql_query('SELECT SUM(roundsplayed) FROM fragstats');
