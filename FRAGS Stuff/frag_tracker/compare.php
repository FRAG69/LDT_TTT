...
$player = mysql_query("SELECT * FROM `fragstats` WHERE `steamid` = '$playerEscaped' LIMIT 0, 30 ");
$player2 = mysql_query("SELECT * FROM `fragstats` WHERE `steamid` = '$playerEscaped2' LIMIT 0, 30 ");
}
elseif($inputType == "NICK") {
$player = mysql_query("SELECT * FROM `fragstats` WHERE `nickname` LIKE '%$playerEscaped%' LIMIT 0, 30 "); //We limit this to 30 based purely on the fact that we'd get way too many results for certain searches.
$player2 = mysql_query("SELECT * FROM `fragstats` WHERE `nickname` LIKE '%$playerEscaped2%' LIMIT 0, 30 "); //We limit this to 30 based purely on the fact that we'd get way too many results for certain searches.
}
else{
$player = mysql_query("SELECT * FROM `fragstats` WHERE `steamid` = '$playerEscaped' LIMIT 0, 30 "); //default to steamid, we should never be here.
$player2 = mysql_query("SELECT * FROM `fragstats` WHERE `steamid` = '$playerEscaped2' LIMIT 0, 30 "); //default to steamid, we should never be here.
}
mysql_close($connect);
}
?>
