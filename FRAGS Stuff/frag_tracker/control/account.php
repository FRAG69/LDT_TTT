...
mysql_query("UPDATE `fragstats`.`admin_users` SET `steamID` = '$plySteamID' WHERE `admin_users`.`user` = '$myUsername'");
}
...
if ($users == 1){
mysql_query("UPDATE `fragstats`.`admin_users` SET `pass` =MD5('$newPass') WHERE `admin_users`.`user` = '$myUsername'");
...
if ($usersDel == 1){
mysql_query("DELETE FROM `fragstats`.`admin_users` WHERE `admin_users`.`user` = '$myUsername'");
include("./logout.php");
}
else{
	echo "<script LANGUAGE='JavaScript'>";
