...
if ($usersActive==1){
$update = mysql_query("UPDATE `fragstats`.`admin_users` SET `last_login` = now(), `last_ip` = '$user_ip' WHERE `admin_users`.`user` = '$myusername'");
$_SESSION['myusername'] = $myusername;
$_SESSION['steamid'] = $plySteamID;
$_SESSION['isadmin'] = $plyAdmin;
header('Location: http://' . $_SERVER['HTTP_HOST'] . '/frag_tracker/login-success.php');
}
else {
