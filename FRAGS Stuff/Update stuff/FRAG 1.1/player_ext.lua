...
function plymeta:RecordKill(victim, dmginfo, headshot, dist)
   ...
   if not util.WeaponFromDamage(dmginfo) then return end
   if not headshot then headshot = false end
   if not dist then dist = -1 end
   
   if not self.LDTAchiveKills then
      self.LDTAchiveKills = {}
   end
   table.insert(self.LDTAchiveKills, {victim:UniqueID(),util.WeaponFromDamage(dmginfo):GetClass(),headshot, dist} )
   
   //Client deals with achivements
   //So send them the data
   umsg.Start("LDTAchUpdKill",self)
      umsg.String(victim:UniqueID())
      umsg.String(util.WeaponFromDamage(dmginfo):GetClass())
	  umsg.Bool(headshot)
	  umsg.Long(dist)
   umsg.End("LDTAchUpdKill")
end


function plymeta:SetSpeed(slowed)
	if self:HasWeapon("weapon_ttt_run") then
		if slowed then
